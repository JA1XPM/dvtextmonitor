...
現在、RS-MS1A / RS-MS1Iはスマホ用アプリしかなく、Windows用ソフトがあればそれらと交信できると考えて開発しました。
...
- log は相対パス/絶対パスどちらでも指定可能です。
  例：python dvtextmonitor.py C:\temp\dvlog.txt
...
--------------------------------------------------------------------------------
7. 免責
本ソフトは無保証です。使用は自己責任でお願いします。
電波法・各種ルールを守って運用してください。

--------------------------------------------------------------------------------
8. ライセンス
本ソフトは MIT License で公開します。詳細は LICENSE を参照してください。
--------------------------------------------------------------------------------

================================================================================
技術解説（RS-MS1A互換パケット構造）
================================================================================
...
B-2) 送信側定数 K の計算（MY → K）
    S = sum(ASCII(MY_BASE)) mod 256
    t = ASCII(lower(MY_SUFFIX)) (suffixがあれば) else 0
    K = (S + 0x1A + t) mod 256

B-3) 宛先側 nn の計算（UR → nn）
    U    = sum(ASCII(UR_BASE)) mod 256
    base = (U + K) mod 256
    nn   = base + ASCII(lower(UR_SUFFIX)) (suffixがあれば) を mod 256
    suffixが無ければ nn = base
...
--------------------------------------------------------------------------------
C) テキスト本文はUTF-8コード、ただしエスケープあり
テキスト本文はUTF-8コードを使用します。しかし本文バイト列中の"使ってはいけないD-STAR予約値"を送信時に置換（エスケープ）する必要があります。
"予約値"(文末参照)とはD-STAR仕様書で決められています。UTF-8漢字コードにこの特定値が現れる事があり、そのままでは通せません。
このエスケープ方式の開始値として0xEFを使用します。そのため本文中の本来の 0xEF は 0xEF 0x6F に変換して区別します。

C-1) 本文エスケープ規則

送信時は、対象バイトの前にエスケープコード 0xEF を付け、対象バイトの最上位ビットを0にした値（最上位ビットを落とした値）を続けます。

- 0xE7 → 0xEF 0x67
  （0x67 は 0xE7 の最上位ビットを0にした値）
- 0xEF → 0xEF 0x6F
  （0x6F は 0xEF の最上位ビットを0にした値）
...
- CSが 0xEF だった場合、それはUTF-8のエスケープに見えてしまい、受信側で次の1バイトを期待するロジックが動作すると考えられます。
そのため、特例として2バイト化して送る必要があります。
- 0x2C は ASCII の ','（カンマ）で、$$Msg の区切り文字でもあるため、実装上都合が悪い可能性があります。
重要：確実に言えるのは「RS-MS1Aはその値のときチェックサムを2バイト化して出してくる」という事実で、理由はあくまで推定です。
...
本文 "text" を送る場合の手順：
  ...
  7) 最終組み立て(python) packet = b"$$Msg," + MY + b"," + UR + b"," + BODY + b"\r\x00"
